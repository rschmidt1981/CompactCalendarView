...
public class CompactCalendarView extends View {
	...
	private int mMaxMonths = 0;

	public void setMaxMonths(int maxMonths) {
		mMaxMonths = maxMonths;
	}
	...
	public CompactCalendarView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		compactCalendarController = new CompactCalendarController(new Paint(), new OverScroller(getContext()),
				...
				Color.argb(255, 219, 219, 219), VelocityTracker.obtain(), Color.argb(255, 100, 68, 65),
	...
	public void setNoEventDayTextColor(int noEventDayTextColor) {
		compactCalendarController.setNoEventDayTextColor(noEventDayTextColor);
		invalidate();
	}
	...
	/**
	 * see {@link #addEvent(Event, boolean)} when adding single events to control if calendar should redraw or {@link
	 * #addEvents(java.util.List)}  when adding multiple events
	 *
	 * @param event
	 */
	...
	/**
	 * Adds an event to be drawn as an indicator in the calendar. If adding multiple events see {@link
	 * #addEvents(List)}} method.
	 *
	 * @param event
	 * 		to be added to the calendar
	 * @param shouldInvalidate
	 * 		true if the view should invalidate
	 */
	...
	/**
	 * see {@link #removeEvent(Event, boolean)} when removing single events to control if calendar should redraw or
	 * {@link #removeEvents(java.util.List)} (java.util.List)}  when removing multiple events
	 *
	 * @param event
	 */
	...
	/**
	 * Removes an event from the calendar. If removing multiple events see {@link #removeEvents(List)}
	 *
	 * @param event
	 * 		event to remove from the calendar
	 * @param shouldInvalidate
	 * 		true if the view should invalidate
	 */
	...
	private void checkTargetHeight() {
		if (compactCalendarController.getTargetHeight() <= 0) {
			throw new IllegalStateException(
					"Target height must be set in xml properties in order to expand/collapse CompactCalendar.");
		}
	}
	...
	/**
	 * Moves the calendar to the right. This will show the next month when {@link #setIsRtl(boolean)} is set to false.
	 * If in rtl mode, it will show the previous month.
	 */
